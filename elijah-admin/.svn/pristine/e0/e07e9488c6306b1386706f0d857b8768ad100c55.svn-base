...
export default {
  name: 'AddDevice',
  props: ['depttreedata', 'fencelist', 'devicefrom'],
  data() {
    return {
      addDeviceFrom: {
        id: '',
  ...
  created() {
    this.addDeviceFrom.id = this.devicefrom.id
    this.addDeviceFrom.code = this.devicefrom.code
    this.addDeviceFrom.department_code = this.devicefrom.department_code
    this.addDeviceFrom.police_car_number = this.devicefrom.police_car_number
    this.stpcode = this.devicefrom.police_code
    var Enclosureclient = []
    for (const i of this.devicefrom.array) {
      Enclosureclient.push(i.id)
    }
    this.Enclosureclient = Enclosureclient
    this.addDeviceFrom.Enclosureclient = this.Enclosureclient.join(',')

    var department_codeArr = []
    for (const i of this.depttreedata) {
      if (i.departmentCode === this.addDeviceFrom.department_code) {
        department_codeArr.push(i.departmentCode)
      } else {
        for (const j of i.children) {
          if (j.departmentCode === this.addDeviceFrom.department_code) {
            department_codeArr.push(i.departmentCode)
            department_codeArr.push(j.departmentCode)
          } else {
            for (const k of j.children) {
              if (k.departmentCode === this.addDeviceFrom.department_code) {
                department_codeArr.push(i.departmentCode)
                department_codeArr.push(j.departmentCode)
                department_codeArr.push(k.departmentCode)
              }
            }
          }
        }
    ...
    this.department_codeArr = department_codeArr
  },
  methods: {
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$post('Client/update', this.addDeviceFrom).then(json => {
            this.$notify({
              title: '成功',
              message: '操作成功！',
              type: 'success'
    ...
    getPoliceList() {
      ...
      this.$post('Department/dptList', {
        ...
        if (this.stpcode !== '') {
          this.addDeviceFrom.police_code = this.stpcode
          this.stpcode = ''
        }
      })
    },
