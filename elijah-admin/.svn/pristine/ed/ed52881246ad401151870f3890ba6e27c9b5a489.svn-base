  ...
  <div>
    ...
    <el-form ref="form" :model="addDataDictFrom" label-width="80px" :rules="rules">
      ...
      <el-form-item label="类型含义" prop="hy">
        <el-input v-model="addDataDictFrom.hy" placeholder="请输入类型含义" />
...
export default {
  ...
  created() {
    this.addDataDictFrom = Object.assign(
      this.addDataDictFrom,
      this.datadictfrom
    )
  },
  methods: {
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$post('DataDict/update', this.addDataDictFrom).then(json => {
            this.$notify({
              title: '成功',
              message: '操作成功！',
              type: 'success'
