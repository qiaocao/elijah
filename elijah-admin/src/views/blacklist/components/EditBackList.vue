...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        typeid: [{ required: true, message: '请输入车牌类型', trigger: 'blur' }]
      },
      platetypeoptions: [
        {
          value: '01',
  ...
  created() {
    this.addBlackListFrom = Object.assign(
      this.addBlackListFrom,
      this.blacklistfrom
    )
  },
  methods: {
    save() {
      this.$refs.form.validate(valid => {
        if (valid) {
          this.$post('BlackList/update', this.addBlackListFrom).then(json => {
            this.$notify({
              title: '成功',
              message: '操作成功！',
              type: 'success'
