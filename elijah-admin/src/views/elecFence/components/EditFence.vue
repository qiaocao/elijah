<template>
  <div>
    ...
    >
      ...
      <el-form-item label="围栏坐标" style="flex:1">
      ...
      <el-form-item>
        ...
        <el-button type="danger" @click="delFence">删除</el-button>
      </el-form-item>
    </el-form>
  </div>
</template>
...
export default {
  ...
  props: {
    ...
    fencedata: { type: Object, required: true, default: () => {} }
  },
  data() {
    return {
      rules: {
  ...
  created() {
    const BMap = this.bmap;
    this.addForm.id = this.fencedata.id;
    this.addForm.name = this.fencedata.Name;
    this.addForm.comment = this.fencedata.Comment;
    this.addForm.region = this.fencedata.Region;
    for (const po of this.addForm.region.split("|")) {
      var llarr = po.split(",");
      this.RegionArr.push([Number(llarr[0]), Number(llarr[1])]);
    }
    // 自动调整位置
    var points = [];
    for (let i = 0; i < this.RegionArr.length; i++) {
      var point = new BMap.Point(
        Number(this.RegionArr[i][0]),
        Number(this.RegionArr[i][1])
      );
      points.push(point);
    }
  ...
  methods: {
    ...
    save() {
      ...
      this.$refs.addForm.validate(valid => {
        if (valid) {
          this.$post("Enclosure/update", this.addForm).then(json => {
            this.$notify({
              title: "成功",
              message: "操作成功！",
              type: "success"
    ...
    delFence() {
      this.$confirm("确认删除该条记录？", "确认信息", {
        ...
        .then(() => {
          this.$post("Enclosure/delete", { id: this.addForm.id }).then(json => {
            this.$notify({
              title: "成功",
              message: "操作成功！",
              type: "success"
